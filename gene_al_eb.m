function [al_eb] = gene_al_eb(reso,angu_size,lmax,cl_eeg,cl_ee,cl_bbg,cl_bb,cl_enp,cl_bnp)
    ...
    for j=1:reso/2+1
        ...
        cosphi_m(j,i)=cos(phi);
         ...
         if (ll<=lmax)
            ...
            cl_bbm1(j,i)=cl_bbg(round(ll)+1)^2 ...
                /(cl_bb(round(ll)+1)+cl_bnp(round(ll)+1));
            cl_eem1(j,i)=1/(cl_ee(round(ll)+1)+cl_enp(round(ll)+1));   
         end
    end
end
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% e11s1_l=
% 1/(cl_ee+cl_np)*lx^2*sin(2phi)^2
e11s1_l=cl_eem1.*lx_m.^2.*sin2phi_m.^2;
e11s1_l(reso/2+1,reso/2+1)=0;
e11s1_l(reso/2+2:reso,:)=e11s1_l(reso/2:-1:2,:);
e11s1=ifft2(ifftshift(e11s1_l))./(delta_x^2);
e11s1(reso/2+1,reso/2+1)=0;
e11s1(1,1)=real(e11s1(1,1));
e11s1(1,reso/2+1)=real(e11s1(1,reso/2+1));
e11s1(reso/2+1,1)=real(e11s1(reso/2+1,1));
e11s1(reso/2+2:reso,1)=conj(e11s1(reso/2:-1:2,1));
e11s1(1,reso/2+2:reso)=conj(e11s1(1,reso/2:-1:2));
e11s1(reso/2+1:reso,reso/2+1:reso)=conj(e11s1(reso/2+1:-1:2,reso/2+1:-1:2));
e11s1(reso/2+2:reso,2:reso/2)=conj(e11s1(reso/2:-1:2,reso:-1:reso/2+2));
% %--------------------------------------------------------------------------
% % e22s_l=
...
%--------------------------------------------------------------------------
% e11c1_l=
% 1/(cl_ee+cl_np)*lx^2*cos(2phi)^2
e11c1_l=cl_eem1.*lx_m.^2.*cos2phi_m.^2;
e11c1_l(reso/2+1,reso/2+1)=0;
e11c1_l(reso/2+2:reso,:)=e11c1_l(reso/2:-1:2,:);
e11c1=ifft2(ifftshift(e11c1_l))./(delta_x^2);
e11c1(reso/2+1,reso/2+1)=0;
e11c1(1,1)=real(e11c1(1,1));
e11c1(1,reso/2+1)=real(e11c1(1,reso/2+1));
e11c1(reso/2+1,1)=real(e11c1(reso/2+1,1));
e11c1(reso/2+2:reso,1)=conj(e11c1(reso/2:-1:2,1));
e11c1(1,reso/2+2:reso)=conj(e11c1(1,reso/2:-1:2));
e11c1(reso/2+1:reso,reso/2+1:reso)=conj(e11c1(reso/2+1:-1:2,reso/2+1:-1:2));
e11c1(reso/2+2:reso,2:reso/2)=conj(e11c1(reso/2:-1:2,reso:-1:reso/2+2));
% %--------------------------------------------------------------------------
% % e22c_l=
...
e11sc1_l=cl_eem1.*lx_m.^2.*sin2phi_m.*cos2phi_m;
e11sc1_l(reso/2+1,reso/2+1)=0;
e11sc1_l(reso/2+2:reso,:)=-e11sc1_l(reso/2:-1:2,:);
e11sc1=ifft2(ifftshift(e11sc1_l))./(delta_x^2);
e11sc1(reso/2+1,reso/2+1)=0;
e11sc1(1,1)=real(e11sc1(1,1));
e11sc1(1,reso/2+1)=real(e11sc1(1,reso/2+1));
e11sc1(reso/2+1,1)=real(e11sc1(reso/2+1,1));
e11sc1(reso/2+2:reso,1)=conj(e11sc1(reso/2:-1:2,1));
e11sc1(1,reso/2+2:reso)=conj(e11sc1(1,reso/2:-1:2));
e11sc1(reso/2+1:reso,reso/2+1:reso)=conj(e11sc1(reso/2+1:-1:2,reso/2+1:-1:2));
e11sc1(reso/2+2:reso,2:reso/2)=conj(e11sc1(reso/2:-1:2,reso:-1:reso/2+2));
%--------------------------------------------------------------------------
% % e22sc_l=
...
%
% b00sc1_l=
% cl_bbg/(cl_bb+cl_np)*sin(2phi)*cos(2phi)
b00sc1_l=cl_bbm1.*cos2phi_m.*sin2phi_m;
b00sc1_l(reso/2+1,reso/2+1)=0;
b00sc1_l(reso/2+2:reso,:)=-b00sc1_l(reso/2:-1:2,:);
b00sc1=ifft2(ifftshift(b00sc1_l))./(delta_x^2);
b00sc1(reso/2+1,reso/2+1)=0;
b00sc1(1,1)=real(b00sc1(1,1));
b00sc1(1,reso/2+1)=real(b00sc1(1,reso/2+1));
b00sc1(reso/2+1,1)=real(b00sc1(reso/2+1,1));
b00sc1(reso/2+2:reso,1)=conj(b00sc1(reso/2:-1:2,1));
b00sc1(1,reso/2+2:reso)=conj(b00sc1(1,reso/2:-1:2));
b00sc1(reso/2+1:reso,reso/2+1:reso)=conj(b00sc1(reso/2+1:-1:2,reso/2+1:-1:2));
b00sc1(reso/2+2:reso,2:reso/2)=conj(b00sc1(reso/2:-1:2,reso:-1:reso/2+2));
%--------------------------------------------------------------------------
% b00c1_l=
% cl_bbg/(cl_bb+cl_np)*cos(2phi)^2
b00c1_l=cl_bbm1.*cos2phi_m.^2;
b00c1_l(reso/2+1,reso/2+1)=0;
b00c1_l(reso/2+2:reso,:)=b00c1_l(reso/2:-1:2,:);
b00c1=ifft2(ifftshift(b00c1_l))./(delta_x^2);
b00c1(reso/2+1,reso/2+1)=0;
b00c1(1,1)=real(b00c1(1,1));
b00c1(1,reso/2+1)=real(b00c1(1,reso/2+1));
b00c1(reso/2+1,1)=real(b00c1(reso/2+1,1));
b00c1(reso/2+2:reso,1)=conj(b00c1(reso/2:-1:2,1));
b00c1(1,reso/2+2:reso)=conj(b00c1(1,reso/2:-1:2));
b00c1(reso/2+1:reso,reso/2+1:reso)=conj(b00c1(reso/2+1:-1:2,reso/2+1:-1:2));
b00c1(reso/2+2:reso,2:reso/2)=conj(b00c1(reso/2:-1:2,reso:-1:reso/2+2));
%--------------------------------------------------------------------------
% b00s1_l=
% cl_bbg/(cl_bb+cl_np)*sin(2phi)^2
b00s1_l=cl_bbm1.*sin2phi_m.^2;
b00s1_l(reso/2+1,reso/2+1)=0;
b00s1_l(reso/2+2:reso,:)=b00s1_l(reso/2:-1:2,:);
b00s1=ifft2(ifftshift(b00s1_l))./(delta_x^2);
b00s1(reso/2+1,reso/2+1)=0;
b00s1(1,1)=real(b00s1(1,1));
b00s1(1,reso/2+1)=real(b00s1(1,reso/2+1));
b00s1(reso/2+1,1)=real(b00s1(reso/2+1,1));
b00s1(reso/2+2:reso,1)=conj(b00s1(reso/2:-1:2,1));
b00s1(1,reso/2+2:reso)=conj(b00s1(1,reso/2:-1:2));
b00s1(reso/2+1:reso,reso/2+1:reso)=conj(b00s1(reso/2+1:-1:2,reso/2+1:-1:2));
b00s1(reso/2+2:reso,2:reso/2)=conj(b00s1(reso/2:-1:2,reso:-1:reso/2+2));
%--------------------------------------------------------------------------
%
%--------------------------------------------------------------------------
g11=e11s.*b00c+e11c.*b00s-2*e11sc.*b00sc;
g111=e11s1.*b00c1+e11c1.*b00s1-2*e11sc1.*b00sc1;
% g22=e22s.*b00c+e22c.*b00s-2*e22sc.*b00sc;
% g12=e12s.*b00c+e12c.*b00s-2*e12sc.*b00sc;
g11_l=real(fftshift(fft2(g11-g111))*delta_x^2); 
% g22_l=real(fftshift(fft2(g22))*delta_x^2);
% g12_l=real(fftshift(fft2(g12))*delta_x^2);
al_ebt= g11_l(1:reso/2+1,:).*lx_m.^2;% ...
%       +g22_l(1:reso/2+1,:).*ly_m.^2 ...
%       +g12_l(1:reso/2+1,:).*lx_m.*ly_m;


al_eb=l_m(reso/2+1,reso/2+1:reso).^2./al_ebt(reso/2+1,reso/2+1:reso);
al_eb(1)=0;
end
